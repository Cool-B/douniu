...
供使用 OpenSpec 进行规范驱动开发的 AI 编码助手使用的指南。
...
- RENAMED：仅当名称更改时使用。如果您也更改行为，请使用 RENAMED（名称）加上引用新名称的 MODIFIED（内容）。

常见陷阱：使用 MODIFIED 添加新关注点而不包括以前的文本。这会导致归档时丢失细节。如果您不是显式更改现有需求，请在 ADDED 下添加新需求。
...
仅在以下情况下增加复杂性：
- 性能数据显示当前解决方案太慢
- 具体规模需求（>1000 用户，>100MB 数据）
- 需要抽象的多个已验证用例

### 清晰引用
- 使用 `file.ts:42` 格式定位代码
- 引用规范如 `specs/auth/spec.md`
- 链接相关变更和 PR
