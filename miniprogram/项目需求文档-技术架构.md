...
| **编程语言**   | TypeScript                 | 类型安全的 JavaScript 超集 |
| **样式预处理** | LESS                       | CSS 预处理器               |
| **网络请求**   | Axios                      | HTTP 客户端库              |
...
3. **可复用性**: 组件可在多个页面中使用
4. **通过事件通信**: 组件通过 triggerEvent 向父组件传递数据
...
export function login(
  ...
  name: string,
  avatar: string
): Promise<ResponseResult<userInfo>>;

// 房间相关
export function createRoom(
  userId: number,
  roomType: number
): Promise<ResponseResult<{ roomInfo: roomInfo }>>;
...
export function addAssistantOrChangeSeat(
  roomId: number,
  seatIndex: number
): Promise<ResponseResult<any>>;
```
